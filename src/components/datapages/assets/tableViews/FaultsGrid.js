...
export const FaultsGrid = ({ faults }) => {
	...
	return (
        <Slide in={faults} direction={'left'}>
		<Grid container justify='center'>
			<Grid container className={classes.headersContainer} justify='center'> 
           ...
           
                {
                    faults.length ? 
                    faults.map((fault,i) => 
                    <Grid container className={classes.rowContainer} justify='center' key={i}> 
                        <Grid item xs={4} className={classes.cellGrid}>
                            <div className={classes.cellData}>
                                <ItemLink itemId={fault.faultId} module={'faults'} size={matches ? 13 : 16} />
                            </div>
                        </Grid>
                        <Grid item xs={4} className={classes.cellGrid}>
                        ...
                        <Grid item xs={4} className={classes.cellGrid}>
                            <div className={clsx(classes.cellData)}>
                                <StatusTag status={fault.status} type='fault' size={matches ? 11 : 13}/>
                            </div>
                        </Grid>
                    </Grid>
                    ) :
